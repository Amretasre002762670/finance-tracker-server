...
app.get('/health', async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: 'Server is healthy and database connection is successful' });
    } catch (error) {
        res.status(500).json({ message: 'Server is healthy but database connection failed' });
    }
});
